...
const App = {
 ...
 getMyBalance: function() {
   ...
   cav.klay.getBalance(walletInstance.address).then(function(balance) {
    $('#my_balance').text(cav.utils.fromPeb(balance,'KLAY'));
   })
 },
 
 getContractBalance: function(){
   contract.methods.getContractBalance().call().then((balance)=>{
    $('#contract_balance').text(balance);
   })
 },

 donate: function(){
   var money = $('#money').val();
   money = cav.utils.toPeb(money,'KLAY');

   var walletInstance = this.getWallet();
   contract.methods.donate(money).send({
